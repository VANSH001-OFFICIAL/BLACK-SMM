...
async def start(update, context):
    if not await is_subscribed(update, context):
        ...
        return await update.message.reply_text("❌ Pehle Join Karein!", reply_markup=InlineKeyboardMarkup(kb))
...
async def order(update, context):
    try:
        ...
        if not bal or bal[0] < total: return await update.message.reply_text(f"❌ Low Balance! Need: {total} RS")
        ...
        else: await update.message.reply_text(f"❌ API Error: {resp.get('error')}")
    except: await update.message.reply_text("Usage: `/order [id] [link] [qty]`", parse_mode='Markdown')

# --- ADD FUND CONVERSATION ---
async def add_fund_start(update, context):
    await update.message.reply_text("UPI: `vansh59rt@fam`\nClick to send Screenshot:", 
...
async def receive_ss(update, context):
    ...
    await context.bot.send_photo(ADMIN_ID, photo, caption=f"💰 Deposit Request\nUser: `{update.effective_user.id}`", 
                                 reply_markup=InlineKeyboardMarkup(admin_kb), parse_mode='Markdown')
    await update.message.reply_text("✅ Sent! Wait for admin approval.")
...
async def approve_cb(update, context):
    ...
    conn.commit()
    await update.callback_query.message.reply_text("✅ Approved 100 RS")
    await context.bot.send_message(uid, "💰 100 RS Added to your account!")
...
if __name__ == '__main__':
    ...
    bot.add_handler(MessageHandler(filters.Regex("^SUPPORT$"), lambda u,c: u.message.reply_text("Contact: @black_Seller16")))
